...
void generate_corr_lines_KL(double* Corr_matrix, int nr_lines, int linelength,
	double *y // out
	)
{
	...
	size_t index=0;
	double * temp; //temporary vector for holding random numbers
	temp=(double*) malloc(sizeof(double)*(linelength));

	//y will have the lines for the turning band method inside of it 
	for (int v = 0; v < nr_lines; v++) {
		for (int i = 0; i < linelength; i++) {	//fill temporary vecor with normally distributed random numbers
			temp[i] = rand_nor_u_var();	//fills temp vector with random numbers of unit variance for KL expansion
		}

		for (int i = 0; i < linelength; i++) {	//This is a very naive matrix multiplication... this part performs the karhuenen loeve expansion because Corr_matrix has the eigenvectors*sqrt(eigenvalues) and temp has random numbers
			y[index] = 0;
			for (int j = 0; j < linelength; j++) {
				y[index]+=Corr_matrix[i*linelength + j] * temp[j];
			}
			index++;	//index goes up for the linelength loop so that the result of the matrix multiplication advances while creating the lines, but index isn't reset for each loop over the number of lines ... so it will fill y with all the lines sequentially
		}
