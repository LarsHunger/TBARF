...
int main(int argc, char *argv[])
{
	...
	for (j = 0; j < size_one_field; j++) {
		...
		Correlation_field[j] /= nr_fields;	//expectation of X_s*X_t
		Correlation_field[j] -= Expectation_field[j] * Expectation_field[ref_point];	//- mu(x)*mu(t)
		Correlation_field[j]/= Variance_field[j] * Variance_field[ref_point];
	}
